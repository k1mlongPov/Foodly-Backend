...
module.exports = {
    createUser: async (req, res) => {
        ...
        if (!emailRegex.test(req.body.email)) {
            return res.status(400).json({ status: false, messaging: "Email is not valid" })
        }
        ...
        try {
            ...
            if (emailExist) {
                return res.status(400).json({ status: false, message: "Email alreadt exist" });
            }
    ...
    loginUser: async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ status: false, message: error.messsage });
        }
    }
}